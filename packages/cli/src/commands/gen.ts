...
export async function genTypes(options: GenTypesOptions) {
  ...
  try {
    ...
    try {
      ...
      let typesContent = `// Generated by basefyio CLI
...
async function generateTsClient(config: any, outputDir: string) {
  const clientContent = `// Generated by basefyio CLI
import axios, { AxiosInstance } from 'axios';

export interface basefyioConfig {
  url?: string;
  anonKey?: string;
  serviceKey?: string;
}
...
export class BasefyioClient {
  ...
  constructor(config: basefyioConfig = {}) {
...
export function createClient(config?: basefyioConfig): BasefyioClient {
  return new BasefyioClient(config);
}
`;

  await fs.mkdir(outputDir, { recursive: true });
  await fs.writeFile(path.join(outputDir, 'basefyio.ts'), clientContent);
}
...
module.exports = { BasefyioClient, createClient };
`;

  await fs.mkdir(outputDir, { recursive: true });
  await fs.writeFile(path.join(outputDir, 'basefyio.js'), clientContent);
}
...
def create_client(config: Optional[Dict[str, str]] = None) -> BasefyioClient:
    return BasefyioClient(config)
`;

  await fs.mkdir(outputDir, { recursive: true });
  await fs.writeFile(path.join(outputDir, 'basefyio.py'), clientContent);
}
