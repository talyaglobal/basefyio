...
export class RealtimeEventsService {
  ...
  /**
   * Realtime is on by default. The legacy BF_REALTIME_PHASE1=1 opt-in remains
   * recognised for backwards compatibility with older deploy configs, but the
   * single source of truth is now BF_REALTIME_DISABLE — set that to '1' to
   * revert to the polling fallback without redeploying.
   */
  private isEnabled() {
    if (process.env.BF_REALTIME_DISABLE === '1') return false;
    return true;
  }
