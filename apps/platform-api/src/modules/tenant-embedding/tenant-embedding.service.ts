...
export class TenantEmbeddingService {
  ...
  async store(
    ...
    try {
      ...
      const existing = await pool.query<{ id: string }>(
        `SELECT id FROM bf_embeddings WHERE content_hash = $1 AND namespace = $2`,
        [hash, namespace],
      );
      if (existing.rows.length > 0) {
        const row = await pool.query<any>(
          `SELECT id, content_hash, namespace, content, metadata, token_count, created_at
           FROM bf_embeddings WHERE id = $1`,
          [existing.rows[0].id],
        );
      ...
      const insertResult = await pool.query<any>(
        `INSERT INTO bf_embeddings (content_hash, namespace, content, metadata, token_count)
         VALUES ($1, $2, $3, $4, $5)
      ...
      // Insert vector
      await pool.query(
        `INSERT INTO bf_embeddings_store (id, embedding)
         VALUES ($1, $2::vector)
  ...
  async search(
    ...
    try {
      ...
      const sql = `
        ...
        FROM bf_embeddings_store es
        JOIN bf_embeddings e ON e.id = es.id
  ...
  async deleteByIds(projectId: string, ids: string[]): Promise<number> {
    ...
    try {
      const result = await pool.query(
        `DELETE FROM bf_embeddings WHERE id = ANY($1::uuid[])`,
        [ids],
      );
  ...
  async deleteByNamespace(
    ...
    try {
      const result = await pool.query(
        `DELETE FROM bf_embeddings WHERE namespace = $1`,
        [namespace],
      );
      return result.rowCount ?? 0;
    } finally {
    ...
    if (project.pgvectorEnabled) {
      ...
      try {
        const result = await pool.query<{ count: string }>(
          `SELECT COUNT(*) AS count FROM bf_embeddings`,
        );
