...
// Recipients for new-feedback notifications. Configurable via env (comma-separated);
// defaults to the public support inbox so no personal addresses live in source.
const NOTIFY_EMAILS = (process.env.FEEDBACK_NOTIFY_EMAILS || 'support@talyasmart.com')
  .split(',')
  .map((s) => s.trim())
  .filter(Boolean);
