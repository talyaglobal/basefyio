export default () => ({
  ...
  docker: {
    ...
    /**
     * Reverse-DNS prefix for the metadata labels we attach to managed Docker
     * resources. Parameterized so a fresh self-hosted install can use its own
  ...
  /**
   * OAuth client id of the platform (control-plane) app inside the Keycloak
   * master realm. Parameterized for self-hosting; default matches the existing
   * deployment so production keeps working unchanged.
   */
  platformKeycloakClientId: process.env.PLATFORM_KEYCLOAK_CLIENT_ID || 'kolaybase-platform',
