...
export const BASEFYIO_AI_SEND_EVENT = 'basefyio-ai-send';

export type BasefyioAiSendDetail = {
  message: string;
  mode?: 'ask' | 'plan' | 'agent';
};

export function dispatchbasefyioAiMessage(detail: BasefyioAiSendDetail) {
  if (typeof window === 'undefined') return;
  window.dispatchEvent(new CustomEvent(BASEFYIO_AI_SEND_EVENT, { detail }));
}
...
export function buildSupabaseImportIssuePrompt(params: {
  ...
  switch (params.kind) {
    ...
    case 'auth_skipped':
      return `${header}${params.detail}\n\nExplain why basefyio/Keycloak might skip auth users during import and what to check or change (email, duplicates, Keycloak config).`;
  }
}
